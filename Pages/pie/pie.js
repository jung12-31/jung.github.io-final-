// ==================== 評論系統功能 ====================
let currentReviewRating = 0; // 儲存當前評分
let isInitialized = false; // 防止重複初始化

// 初始化評論系統
function initReviewSystem() {
    if (isInitialized) {
        console.log('評論系統已初始化，跳過重複初始化');
        return;
    }
...
function initializeReviewsData() {
    const reviewsData = localStorage.getItem('chulinReviews');
    
    if (!reviewsData) {
        console.log('沒有找到評論資料，創建預設資料');
        const defaultReviews = createDefaultReviews();
        localStorage.setItem('chulinReviews', JSON.stringify(defaultReviews));
        console.log('預設評論已創建:', defaultReviews);
    } else {
        console.log('已有評論資料:', JSON.parse(reviewsData));
    }
}
...
function createDefaultReviews() {
    return {
        '蘋果派': {
            rating: 4.7,
            reviews: [
                { name: '王小姐', rating: 5, text: '蘋果內餡酸甜適中，塔皮酥脆不油膩，家人非常喜歡！送貨速度也很快，包裝很仔細，會再回購！', date: '2024-03-15' },
                { name: '李先生', rating: 4, text: '味道很不錯，不過希望可以再大一點點。整體來說CP值很高，會推薦給朋友！', date: '2024-03-10' },
                { name: '張太太', rating: 5, text: '派皮做得很好，酥脆又不油膩。蘋果餡料也很實在，吃得到蘋果塊！', date: '2024-03-05' }
            ]
        },
        '櫻桃派': {
            rating: 4.3,
            reviews: [
                { name: '陳太太', rating: 5, text: '櫻桃的酸甜恰到好處，女兒生日指定要這款！大家都說好吃。', date: '2024-03-12' },
                { name: '張先生', rating: 4, text: '很好吃，不會太甜，適合不愛吃太甜的人。包裝精緻，送禮很適合。', date: '2024-03-08' }
            ]
        },
        '樹莓派': {
            rating: 4.5,
            reviews: [
                { name: '林小姐', rating: 5, text: '酸甜比例完美，樹莓的香氣很迷人！冷凍後再吃也別有風味。', date: '2024-03-14' },
                { name: '趙先生', rating: 4, text: '送禮自用兩相宜，朋友都說好吃。還會再回購！', date: '2024-03-09' }
            ]
        },
        '胡桃派': {
            rating: 4.9,
            reviews: [
                { name: '吳女士', rating: 5, text: '胡桃香氣十足，口感豐富，是成熟大人的口味！用料非常實在。', date: '2024-03-16' },
                { name: '鄭先生', rating: 4, text: '用料實在，每一口都能吃到滿滿的胡桃。稍微甜一點，但很美味。', date: '2024-03-11' }
            ]
        },
        '檸檬蛋白派': {
            rating: 4.6,
            reviews: [
                { name: '周小姐', rating: 5, text: '檸檬的酸度很剛好，蛋白霜入口即化，超喜歡！夏天吃很清爽。', date: '2024-03-13' },
                { name: '黃先生', rating: 4, text: '夏天吃很清爽，不會覺得膩。蛋白霜打得很好，口感輕盈。', date: '2024-03-07' }
            ]
        }
    };
}
...
function loadProductReviews() {
    try {
        const reviewsData = localStorage.getItem('chulinReviews');
        if (!reviewsData) {
            console.error('沒有找到評論資料');
            return;
        }
        
        const reviews = JSON.parse(reviewsData);
        console.log('載入評論資料:', reviews);
        
        document.querySelectorAll('.tart-card').forEach(card => {
            const productName = card.querySelector('.tart-name').textContent;
            const productReviews = reviews[productName];
            
            console.log(`處理 ${productName}:`, productReviews);
            
            if (productReviews) {
                // **修正：確保 rating 是正確的數字格式**
                ...
                // 更新星星顯示
                if (ratingStars) {
                    updateProductStarsDisplay(ratingStars, rating);
                }
                
                // 更新評論預覽
                if (productReviews.reviews) {
                    updateReviewsPreview(card, productReviews.reviews);
                }
            }
        });
        
        console.log('評論資料載入完成');
    } catch (error) {
        console.error('載入評論資料時發生錯誤:', error);
    }
}

// 在 pie.html 的評論顯示函數中
function updateReviewsPreview(card, reviews) {
...
// 更新商品星星顯示
function updateProductStarsDisplay(starsElement, rating) {
...
function setupReviewButtons() {
    console.log('設置評論按鈕事件...');
    
    // 為所有「我要評分」按鈕綁定事件
    const rateButtons = document.querySelectorAll('.rate-product-btn');
    console.log('找到我要評分按鈕:', rateButtons.length);
    ...
    rateButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const card = this.closest('.tart-card');
            if (!card) {
                console.log('找不到卡片元素');
                return;
            }
            ...
            const productName = card.querySelector('.tart-name').textContent;
            console.log('點擊我要評分:', productName);
            
            // 開啟評分模態框
            openRatingModalWithAuth(productName);
        });
    });
    
    // 為所有「查看評論」按鈕綁定事件
    const viewAllButtons = document.querySelectorAll('.view-all-reviews-btn');
    console.log('找到查看評論按鈕:', viewAllButtons.length);
    ...
    viewAllButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const productName = card.querySelector('.tart-name').textContent;
            console.log('點擊查看評論:', productName);
            
            // 開啟評論模態框
            openReviewsModal(productName);
        });
    });
}

// 開啟評分模態框（整合會員登入檢查）
function openRatingModalWithAuth(productName) {
    console.log('開啟評分模態框:', productName);
    ...
    if (!isLoggedIn || !userData.email) {
        ...
        if (authModal) {
            ...
            setTimeout(() => {
                ...
                localStorage.setItem('pendingRatingProduct', productName);
            }, 10);
        }
        return;
    }
...
function initRatingModalStars() {
    ...
    stars.forEach(star => {
        ...
        star.addEventListener('mouseleave', function() {
            highlightStars(stars, currentReviewRating);
            updateRatingText(ratingText, currentReviewRating);
        });
...
// 更新評分文字
function updateRatingText(element, rating) {
    if (!element) return;
...
function openReviewsModal(productName) {
    ...
    console.log('開啟評論模態框:', productName);
    ...
    if (productReviews) {
        ...
        // 計算評分分佈
        const distribution = calculateRatingDistribution(productReviews.reviews);
        ...
        // 更新星星顯示
        if (ratingStarsLarge) {
            ratingStarsLarge.innerHTML = getStarsHTML(productReviews.rating);
        }
        
        // **修正：更新評分分佈圖**
        if (ratingDistribution) {
            updateRatingDistributionDisplay(ratingDistribution, distribution);
        }
...
function calculateRatingDistribution(reviews) {
    ...
    reviews.forEach(review => {
        const rating = Math.floor(review.rating); // 取整數星等
...
// **更新評分分佈顯示的函數**
function updateRatingDistributionDisplay(container, distribution) {
    if (!container) return;
...
function submitRating() {
    ...
    console.log('提交評分，產品:', productName, '評分:', currentReviewRating);
    ...
    try {
        ...
        console.log('是否會員:', isLoggedIn);
        
        // 確保用戶資料有正確的值
        const userEmail = userData.email || '';
        const userName = userData.name || userData.email?.split('@')[0] || '會員';
        
        console.log('將要儲存的用戶資訊:');
        console.log('  userEmail:', userEmail);
        console.log('  userName:', userName);
        
        // 獲取現有評論資料
        const reviewsData = localStorage.getItem('chulinReviews');
        let reviews = reviewsData ? JSON.parse(reviewsData) : createDefaultReviews();
        ...
        // **新增：儲存評分分佈資料**
        reviews[productName].ratingDistribution = calculateRatingDistribution(productReviews);
        
        // 儲存到 localStorage
        localStorage.setItem('chulinReviews', JSON.stringify(reviews));
        
        console.log('評論已儲存，更新後的商品資料:', reviews[productName]);
        
        // **修正：如果評論模態框已開啟，更新顯示**
        const reviewsModal = document.getElementById('reviewsModal');
        const currentModalProduct = reviewsModal ? 
            document.getElementById('reviewsModalTitle')?.textContent.replace(' - 商品評論', '') : '';
        
        if (reviewsModal && reviewsModal.style.display === 'flex' && currentModalProduct === productName) {
            // 直接更新模態框內容，而不是重新開啟
            updateReviewsModalContent(productName, reviews[productName]);
        } else {
            // 更新UI
            loadProductReviews();
        }
...
// **更新評論模態框內容的函數（不重新開啟）**
function updateReviewsModalContent(productName, productReviews) {
    ...
    // 計算或取得評分分佈
    const distribution = productReviews.ratingDistribution || 
                        calculateRatingDistribution(productReviews.reviews);
    ...
    console.log('評論模態框內容已更新');
}

// **初始化預設評分分佈資料**
function initializeDefaultRatingDistributions() {
    const reviewsData = localStorage.getItem('chulinReviews');
    ...
    Object.keys(reviews).forEach(productName => {
        const productData = reviews[productName];
        
        if (productData && productData.reviews && !productData.ratingDistribution) {
            // 計算評分分佈
            productData.ratingDistribution = calculateRatingDistribution(productData.reviews);
            updated = true;
        }
    });
    ...
    if (updated) {
        localStorage.setItem('chulinReviews', JSON.stringify(reviews));
        console.log('已初始化所有商品的評分分佈資料');
    }
}

// **清理重複評論的函數**
function cleanDuplicateReviews() {
    try {
        const reviewsData = localStorage.getItem('chulinReviews');
        if (!reviewsData) return;
        ...
        Object.keys(reviews).forEach(productName => {
            ...
            if (productData && productData.reviews && Array.isArray(productData.reviews)) {
                const uniqueReviews = [];
                const seenReviews = new Set();
                ...
                // 更新為去重後的評論
                if (productData.reviews.length !== uniqueReviews.length) {
                    productData.reviews = uniqueReviews;
                    
                    // 重新計算評分
                    const totalRating = uniqueReviews.reduce((sum, review) => {
                        const rating = Number(review.rating) || 0;
                        return sum + rating;
                    }, 0);
                    
                    productData.rating = uniqueReviews.length > 0 ? 
                        parseFloat((totalRating / uniqueReviews.length).toFixed(1)) : 0;
                }
            }
        });
        ...
        if (cleanedCount > 0) {
            localStorage.setItem('chulinReviews', JSON.stringify(reviews));
            console.log(`已清理 ${cleanedCount} 則重複評論`);
            showNotification(`已清理 ${cleanedCount} 則重複評論`, 'info');
        }
        
        return reviews;
    } catch (error) {
        console.error('清理重複評論時發生錯誤:', error);
        return null;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('頁面加載完成，初始化評論系統...');
    
    // 先清理重複評論
    cleanDuplicateReviews();

    // 初始化評分分佈資料
    initializeDefaultRatingDistributions();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        // 處理評論模態框外部點擊關閉
        const reviewsModal = document.getElementById('reviewsModal');
        if (reviewsModal && reviewsModal.classList.contains('active') && e.target === reviewsModal) {
            closeReviewsModal();
        }
        
        // 處理評分模態框外部點擊關閉
        const ratingModal = document.getElementById('ratingModal');
        if (ratingModal && ratingModal.classList.contains('active') && e.target === ratingModal) {
            closeRatingModal();
        }
    });
...
function checkPendingRating() {
    ...
    if (pendingProduct) {
        ...
        if (isLoggedIn && userData.email) {
            // 如果已登入，自動開啟評分模態框
            console.log('用戶已登入，自動開啟待處理的評分:', pendingProduct);
            setTimeout(() => {
...
// 檢查是否有待編輯的評論
function checkEditingReview() {
    const isEditing = window.location.search.includes('editReview=true');
    const productParam = new URLSearchParams(window.location.search).get('product');
    
    if (isEditing && productParam) {
        const productName = decodeURIComponent(productParam);
        const editingData = JSON.parse(localStorage.getItem('editingReview') || '{}');
        
        if (editingData.productName === productName) {
            // 自動開啟評分模態框並填充資料
            setTimeout(() => {
                openRatingModalWithAuth(productName);
                ...
                setTimeout(() => {
                    ...
                    if (contentInput && editingData.text) {
                        // 嘗試解析標題和內容
                        const text = editingData.text;
                        const dashIndex = text.indexOf(' - ');
                        
                        if (dashIndex > 0) {
                            if (titleInput) titleInput.value = text.substring(0, dashIndex);
                            contentInput.value = text.substring(dashIndex + 3);
                        } else {
                    ...
                    // 設定評分
                    if (editingData.rating) {
                        currentReviewRating = editingData.rating;
                        const starsContainer = document.querySelector('#ratingModal .stars-selector');
...
function initProductButtons() {
    ...
    followBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const productName = card.querySelector('.tart-name').textContent;
            const productPrice = parseInt(card.querySelector('.tart-price').textContent.replace('NT$ ', '').replace(',', ''));
...
function initProductIds() {
    const cards = document.querySelectorAll('.tart-card');
    cards.forEach((card, index) => {
        card.dataset.id = `tart_${index + 1}`;
    });
}
...
function toggleWishlist(productId, productName, productPrice, productImage) {
    ...
    const followBtn = document.querySelector(`.tart-card[data-id="${productId}"] .btn-follow`);
...
function updateFollowButtons() {
    const followBtns = document.querySelectorAll('.btn-follow');
    followBtns.forEach(btn => {
        const card = btn.closest('.tart-card');
        const productId = card.dataset.id;
