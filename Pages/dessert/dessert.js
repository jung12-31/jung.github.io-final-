    ...
    if (!document.getElementById('notification-styles')) {
        ...
        style.textContent = `
            .global-notification {
                ...
                gap: 10px;
                font-weight: 500;
                border-left: 4px solid #2196F3; /* 預設藍色 */
                min-width: 300px;
                max-width: 400px;
            }
...
function showNotification(message, type = 'info') {
    ...
    // 這裡確保 class 名稱與 CSS 對應：notification-success, notification-error, notification-info
...
// 兼容舊的函數名稱
function showBakedNotification(message, type = 'info') {
    showNotification(message, type);
}

function showPuffNotification(message, type = 'info') {
    showNotification(message, type);
}
...
// 初始化西式常溫甜點評論系統
function initBakedReviewSystem() {
    if (isInitialized) {
        console.log('西式常溫甜點評論系統已初始化，跳過重複初始化');
        return;
    }
    
    console.log('初始化西式常溫甜點評論系統');
    
    // 檢查是否有評論資料，如果沒有則創建預設資料
    initializeBakedReviewsData();
    
    // 載入商品評論資料
    loadBakedProductReviews();
    
    // 設置評論按鈕事件
    setupBakedReviewButtons();
    
    // 初始化星星評分系統
    initBakedRatingModalStars();
    
    // 綁定提交按鈕
    bindBakedSubmitButton();
...
// 初始化西式常溫甜點評論資料
function initializeBakedReviewsData() {
    const reviewsData = localStorage.getItem('chulinBakedReviews');
    
    if (!reviewsData) {
        console.log('沒有找到西式常溫甜點評論資料，創建預設資料');
        const defaultReviews = createBakedDefaultReviews();
        localStorage.setItem('chulinBakedReviews', JSON.stringify(defaultReviews));
        console.log('西式常溫甜點預設評論已創建:', defaultReviews);
    } else {
        console.log('已有西式常溫甜點評論資料:', JSON.parse(reviewsData));
    }
}

// 建立西式常溫甜點預設評論資料
function createBakedDefaultReviews() {
    return {
        '司康': {
            rating: 4.5,
            reviews: [
                { name: '陳先生', rating: 5, text: '外層咬下去有很棒的酥脆感，裡面卻不會乾澀，淡淡的奶油香配茶剛剛好。', date: '2024-03-18' },
                { name: '林小姐', rating: 4, text: '低糖配方吃起來沒負擔，夾一點奶油乳酪進去，味道的層次感瞬間提升！', date: '2024-03-20' },
                { name: '王太太', rating: 5, text: '配果醬真的超好吃！外酥內軟的口感很療癒。', date: '2024-03-15' }
            ]
        },
        '可麗露': {
            rating: 4.8,
            reviews: [
                { name: '張小姐', rating: 5, text: '焦糖外殼脆度滿分！內餡濕潤且蘭姆酒香氣很迷人，甜度控制得非常完美。', date: '2024-03-22' },
                { name: '王先生', rating: 5, text: '口感非常有嚼勁，外焦內軟的對比很明顯，是我吃過最道地的法式風味。', date: '2024-03-25' },
                { name: '李太太', rating: 4, text: '蘭姆酒香氣恰到好處，不會太濃烈，吃起來很有層次感。', date: '2024-03-19' }
            ]
        },
        '瑪德蓮': {
            rating: 4.7,
            reviews: [
                { name: '黃小姐', rating: 5, text: '貝殼造型超可愛，口感很細緻，入口即化的奶香味真的非常療癒，下午茶首選。', date: '2024-03-28' },
                { name: '周先生', rating: 4, text: '口感溫潤且不乾口，配咖啡非常適合，是一道吃了會讓人心情變好的甜點。', date: '2024-04-01' },
                { name: '吳小姐', rating: 5, text: '奶香味濃郁但又不會太甜，是完美的下午茶點心！', date: '2024-03-30' }
            ]
        },
        '費南雪': {
            rating: 4.6,
            reviews: [
                { name: '郭小姐', rating: 5, text: '杏仁粉和焦化奶油的組合非常濃郁，外皮帶點微酥，整體紮實卻不厚重。', date: '2024-04-03' },
                { name: '劉小姐', rating: 5, text: '奶油香氣在嘴裡散開的感覺太幸福了！雖然濃醇但吃完不會覺得膩口。', date: '2024-04-05' },
                { name: '鄭先生', rating: 4, text: '口感很紮實，杏仁香氣很足，配茶很適合。', date: '2024-04-01' }
            ]
        },
        '馬卡龍': {
            rating: 4.9,
            reviews: [
                { name: '趙小姐', rating: 5, text: '外殼真的很酥脆，內餡柔軟細滑，重點是降低甜度後果香更明顯了，吃起來完全不膩口！', date: '2024-04-10' },
                { name: '孫先生', rating: 5, text: '色彩繽紛非常吸睛，奶香跟果香平衡得很好，是視覺與味覺的雙重享受，送禮也很體面。', date: '2024-04-12' },
                { name: '錢小姐', rating: 5, text: '各種口味都很好吃，甜度控制得剛剛好，不會像一般馬卡龍那麼甜膩。', date: '2024-04-08' }
            ]
        }
    };
}
...
// 載入西式常溫甜點商品評論資料到頁面
function loadBakedProductReviews() {
    try {
        const reviewsData = localStorage.getItem('chulinBakedReviews');
        if (!reviewsData) {
            console.error('沒有找到西式常溫甜點評論資料');
            return;
        }
        
        const reviews = JSON.parse(reviewsData);
        console.log('載入西式常溫甜點評論資料，共有', Object.keys(reviews).length, '個商品評論');
        
        // 獲取頁面上的所有西式常溫甜點商品卡片
        const bakedCards = document.querySelectorAll('.baked-card');
        console.log('頁面上有', bakedCards.length, '個西式常溫甜點商品卡片');
        
        bakedCards.forEach((card, index) => {
            const productName = card.querySelector('.baked-name').textContent.trim();
            ...
            if (productReviews) {
                ...
                // 更新星星顯示
                if (ratingStars) {
                    updateBakedProductStarsDisplay(ratingStars, rating);
                }
                
                // 更新評論預覽
                if (productReviews.reviews && productReviews.reviews.length > 0) {
                    updateBakedReviewsPreview(card, productReviews.reviews);
                } else {
                    // 如果沒有評論，顯示暫無評論
                ...
                const ratingStars = card.querySelector('.rating-stars');
                if (ratingStars) {
                    updateBakedProductStarsDisplay(ratingStars, 0);
                }
            }
        });
        
        console.log('西式常溫甜點評論資料載入完成');
    } catch (error) {
        console.error('載入西式常溫甜點評論資料時發生錯誤:', error);
    }
}

// 更新西式常溫甜點評論預覽
function updateBakedReviewsPreview(card, reviews) {
    const previewContainer = card.querySelector('.reviews-preview');
            ...
            reviewItem.innerHTML = `
                <div class="reviewer-info">
                    ...
                    <div class="review-stars">
                        ${getBakedStarsHTML(review.rating)}
                    </div>
                </div>
...
// 獲取西式常溫甜點星星HTML
function getBakedStarsHTML(rating) {
    let starsHTML = '';
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
...
// 更新西式常溫甜點商品星星顯示
function updateBakedProductStarsDisplay(starsElement, rating) {
    ...
    for (let i = 0; i < fullStars; i++) {
        ...
        star.style.color = '#ffc107'; // 金色星星
        star.style.textShadow = '0 0 4px rgba(255, 193, 7, 0.3)'; // 輕微發光效果
...
// 設置西式常溫甜點評論按鈕事件
function setupBakedReviewButtons() {
    console.log('設置西式常溫甜點評論按鈕事件...');
    
    // 為所有「我要評分」按鈕綁定事件
    const rateButtons = document.querySelectorAll('.baked-card .rate-product-btn');
    console.log('找到西式常溫甜點我要評分按鈕:', rateButtons.length);
    ...
    rateButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const card = this.closest('.baked-card');
            if (!card) {
                console.log('找不到西式常溫甜點卡片元素');
                return;
            }
            
            const productName = card.querySelector('.baked-name').textContent.trim();
            console.log('點擊西式常溫甜點我要評分:', productName);
            
            // 開啟評分模態框
            openBakedRatingModalWithAuth(productName);
    ...
    // 為所有「查看評論」按鈕綁定事件
    const viewAllButtons = document.querySelectorAll('.baked-card .view-all-reviews-btn');
    console.log('找到西式常溫甜點查看評論按鈕:', viewAllButtons.length);
    ...
    viewAllButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const productName = card.querySelector('.baked-name').textContent.trim();
            console.log('點擊西式常溫甜點查看評論:', productName);
            
            // 開啟評論模態框
            openBakedReviewsModal(productName);
...
// 開啟西式常溫甜點評分模態框（整合會員登入檢查）
function openBakedRatingModalWithAuth(productName) {
    console.log('開啟西式常溫甜點評分模態框:', productName);
    ...
    if (!isLoggedIn || !userData.email) {
        ...
        showBakedNotification('請先登入會員才能進行評分', 'error');
        ...
        if (authModal) {
            ...
            setTimeout(() => {
                ...
                localStorage.setItem('pendingBakedRatingProduct', productName);
            }, 10);
        }
        return;
    }
    ...
    openBakedRatingModal(productName);
}

// 初始化西式常溫甜點評分模態框的星星
function initBakedRatingModalStars() {
    ...
    stars.forEach(star => {
        ...
        star.addEventListener('mouseenter', function() {
            ...
            highlightBakedStars(stars, value);
            updateBakedRatingText(ratingText, value);
        });
        
        // 滑鼠移出事件 - 恢復實際評分
        star.addEventListener('mouseleave', function() {
            highlightBakedStars(stars, currentReviewRating);
            updateBakedRatingText(ratingText, currentReviewRating);
        });
        
        // 點擊事件 - 設定評分（移除 e.stopPropagation()）
    ...
    // 初始化顯示
    highlightBakedStars(stars, 0);
    updateBakedRatingText(ratingText, 0);
}

// 高亮西式常溫甜點星星
function highlightBakedStars(stars, rating) {
...
// 更新西式常溫甜點評分文字
function updateBakedRatingText(element, rating) {
    if (!element) return;
...
// 開啟西式常溫甜點評論模態框
function openBakedReviewsModal(productName) {
    ...
    console.log('開啟西式常溫甜點評論模態框:', productName);
    ...
    if (productReviews) {
        ...
        // 更新星星顯示
        if (ratingStarsLarge) {
            ratingStarsLarge.innerHTML = getBakedStarsHTML(productReviews.rating);
        }
...
// 關閉西式常溫甜點評論模態框
function closeBakedReviewsModal() {
    const modal = document.getElementById('reviewsModal');
    if (modal) {
...
// 開啟西式常溫甜點評分模態框
function openBakedRatingModal(productName) {
...
// 關閉西式常溫甜點評分模態框
function closeBakedRatingModal() {
    const modal = document.getElementById('ratingModal');
    if (modal) {
        modal.classList.remove('active');
        setTimeout(() => {
...
// 綁定西式常溫甜點提交按鈕
function bindBakedSubmitButton() {
...
function bindBreadCancelButton() {
    ...
    if (cancelBtn) {
        ...
        newCancelBtn.addEventListener('click', function(e) {
            // 只阻止預設行為
            e.preventDefault();
            closeBakedRatingModal(); // 修正函數名稱
        });
        
        console.log('西式常溫甜點取消按鈕已重新綁定');
    }
}

// 提交西式常溫甜點評分
function submitBakedRating() {
    const modal = document.getElementById('ratingModal');
    if (!modal) {
    ...
    console.log('提交西式常溫甜點評分，產品:', productName, '評分:', currentReviewRating);
    
    // 驗證評分
    if (currentReviewRating === 0) {
        showBakedNotification('請選擇評分星等', 'error');
        return;
    }
    ...
    // 驗證評論內容
    if (!content) {
        showBakedNotification('請填寫評論內容', 'error');
        return;
    }
    ...
    try {
        ...
        // 獲取現有評論資料
        const reviewsData = localStorage.getItem('chulinBakedReviews');
        let reviews = reviewsData ? JSON.parse(reviewsData) : createBakedDefaultReviews();
        ...
        // 儲存到 localStorage
        localStorage.setItem('chulinBakedReviews', JSON.stringify(reviews));
        
        console.log('西式常溫甜點評論已儲存，更新後的商品資料:', reviews[productName]);
        
        // 更新UI
        loadBakedProductReviews();
        
        // 顯示成功訊息
        showBakedNotification('評論提交成功！感謝您的分享', 'success');
        
        // 關閉模態框
        closeBakedRatingModal();
        
    } catch (error) {
        console.error('提交西式常溫甜點評分時發生錯誤:', error);
        showBakedNotification('提交評分時發生錯誤，請稍後再試', 'error');
    }
}
...
function showBakedNotification(message, type = 'info') {
    // 移除現有的通知
    const existingNotification = document.querySelector('.baked-notification');
    if (existingNotification) existingNotification.remove();
    
    // 創建通知元素
    const notification = document.createElement('div');
    notification.className = `baked-notification notification ${type}`;
    notification.innerHTML = `
        <div class="notification-content">
...
// 檢查待處理的西式常溫甜點評分請求
function checkPendingBakedRating() {
    const pendingProduct = localStorage.getItem('pendingBakedRatingProduct');
    if (pendingProduct) {
        const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
        const userData = JSON.parse(localStorage.getItem('chulin_user')) || {};
        ...
        if (isLoggedIn && userData.email) {
            // 如果已登入，自動開啟評分模態框
            console.log('用戶已登入，自動開啟待處理的西式常溫甜點評分:', pendingProduct);
            setTimeout(() => {
                openBakedRatingModal(pendingProduct);
                // 清除待處理請求
                localStorage.removeItem('pendingBakedRatingProduct');
            }, 500);
        }
    }
}
...
// 修復西式常溫甜點模態框關閉功能
function fixBakedModalClose() {
    console.log('修復西式常溫甜點模態框關閉功能...');
...
// 添加西式常溫甜點通知樣式
function addBakedNotificationStyles() {
    if (!document.getElementById('baked-notification-styles')) {
        const style = document.createElement('style');
        style.id = 'baked-notification-styles';
        style.textContent = `
            .notification {
                position: fixed;
                top: 20px;
...
// ==================== 西式常溫甜點專區主初始化流程 ====================
function initializeBakedPage() {
    console.log('西式常溫甜點專區頁面加載完成，開始初始化...');
    
    // 初始化西式常溫甜點評論資料
    initializeBakedReviewsData();
    
    // 初始化西式常溫甜點評論系統
    initBakedReviewSystem();
    
    // 修復模態框關閉功能
    fixBakedModalClose();
    
    // 添加通知樣式
    addBakedNotificationStyles();
    
    // 檢查是否有待處理的評分請求
    checkPendingBakedRating();
    
    console.log('西式常溫甜點專區頁面初始化完成');
}

// ==================== DOM 載入完成後初始化西式常溫甜點專區 ====================
document.addEventListener('DOMContentLoaded', function() {
    console.log('西式常溫甜點專區頁面已載入');
...
// ==================== 調試函數 ====================
// 手動重置西式常溫甜點評論資料
function resetBakedReviews() {
    localStorage.removeItem('chulinBakedReviews');
    console.log('西式常溫甜點評論資料已重置');
    location.reload();
}

// 檢查西式常溫甜點當前狀態
function checkBakedReviewsStatus() {
    const reviewsData = localStorage.getItem('chulinBakedReviews');
    console.log('=== 西式常溫甜點評論系統狀態檢查 ===');
    console.log('LocalStorage 中是否有西式常溫甜點評論資料:', reviewsData ? '是' : '否');
    
    if (reviewsData) {
        const reviews = JSON.parse(reviewsData);
        console.log('所有西式常溫甜點商品鍵名:', Object.keys(reviews));
        
        // 檢查每個商品
        document.querySelectorAll('.baked-card').forEach((card, index) => {
            const productName = card.querySelector('.baked-name').textContent.trim();
...
// 將調試函數暴露到全局
window.resetBakedReviews = resetBakedReviews;
window.checkBakedReviewsStatus = checkBakedReviewsStatus;

// 全局函數，讓HTML可以呼叫
window.openBakedReviewsModal = openBakedReviewsModal;
window.closeBakedReviewsModal = closeBakedReviewsModal;
window.openBakedRatingModal = openBakedRatingModal;
window.closeBakedRatingModal = closeBakedRatingModal;
window.submitBakedRating = submitBakedRating;
...
function initProductButtons() {
    ...
    followBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const productName = card.querySelector('.baked-name').textContent .trim();
            const productPrice = parseInt(card.querySelector('.baked-price').textContent.replace('NT$ ', '').replace(',', ''));
...
function initProductIds() {
    const cards = document.querySelectorAll('.baked-card');
    cards.forEach((card, index) => {
        card.dataset.id = `baked_${index + 1}`;
    });
}
...
function toggleWishlist(productId, productName, productPrice, productImage) {
    ...
    const followBtn = document.querySelector(`.baked-card[data-id="${productId}"] .btn-follow`);
...
function updateFollowButtons() {
    const followBtns = document.querySelectorAll('.btn-follow');
    followBtns.forEach(btn => {
        const card = btn.closest('.baked-card');
        const productId = card.dataset.id;
...
function addMessage(text, sender) {
    ...
    const now = new Date();
    const timeString = `${now.getHours().toString().padSbaked(2, '0')}:${now.getMinutes().toString().padSbaked(2, '0')}`;
