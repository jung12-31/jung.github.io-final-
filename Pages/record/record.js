...
if (loginForm) {
    loginForm.addEventListener('submit', function(e) {
        ...
        setTimeout(() => {
            ...
            const deviceName = isMobile ? "行動裝置 / 手機瀏覽器" : "桌上型電腦 / 瀏覽器";
            ...
            // 3. 【關鍵】新增紀錄到近期登入清單
            let loginLogs = JSON.parse(localStorage.getItem('chulin_login_logs')) || [];
            loginLogs.unshift({
                time: timeString,
                device: deviceName,
                status: '成功'
            ...
            // 只保留最近 10 筆
            if (loginLogs.length > 10) loginLogs = loginLogs.slice(0, 10);
            localStorage.setItem('chulin_login_logs', JSON.stringify(loginLogs));

            // 4. 立即更新表格內容 (如果當前頁面有表格的話)
            if (typeof renderLoginLogs === 'function') {
                renderLoginLogs();
            }
...
// 儲存登入紀錄到 localStorage
function saveLoginLog(time, device) {
    let logs = JSON.parse(localStorage.getItem('chulin_login_logs')) || [];
    // 將新紀錄放在最前面
    logs.unshift({ time, device, status: '成功' });
    // 只保留最近 5 筆
    if (logs.length > 5) logs = logs.slice(0, 5);
    localStorage.setItem('chulin_login_logs', JSON.stringify(logs));
}
...
function renderLoginLogs() {
    ...
    // 如果完全沒資料，顯示一筆模擬資料或提示
    if (logs.length === 0) {
        logList.innerHTML = '<tr><td colspan="3" style="text-align:center;">暫無登入紀錄</td></tr>';
        return;
    }
    ...
    logList.innerHTML = logs.map(log => `
        <tr>
            <td>${log.time}</td>
            <td>${log.device}</td>
            <td><span style="color: #28a745; font-weight: bold;">${log.status}</span></td>
        </tr>
    `).join('');
}
