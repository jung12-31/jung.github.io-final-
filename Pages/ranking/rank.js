...
function initProductButtons() {
    console.log('初始化產品按鈕功能');
    
    // 載入喜好清單
    loadWishlist();
    
    // 1. 綁定所有追蹤按鈕
    document.querySelectorAll('.btn-follow').forEach((button, index) => {
        // 清除舊的事件監聽器
    ...
    document.querySelectorAll('.btn-follow').forEach((button, index) => {
        button.addEventListener('click', function(e) {
            ...
            // 找到對應的產品卡片
            const productCard = this.closest('.puff-card');
            if (!productCard) {
            ...
            const productName = productCard.querySelector('.puff-name')?.textContent || '產品';
            const productPriceText = productCard.querySelector('.puff-price')?.textContent || 'NT$ 0';
            const productPrice = parseInt(productPriceText.replace(/[^0-9]/g, '')) || 0;
            const productImage = productCard.querySelector('img')?.src || '';
            
            console.log('產品資訊:', { productId, productName, productPrice, productImage });
    ...
    // 2. 綁定所有加入購物車按鈕
    // 先修復第3名按鈕的圖示問題
    const thirdCartBtn = document.querySelector('.puff-card[data-id="003"] .btn-cart');
    if (thirdCartBtn) {
        const icon = thirdCartBtn.querySelector('i');
    ...
    // 3. 更新所有追蹤按鈕的初始狀態
    updateFollowButtonStates();
    
    console.log('產品按鈕初始化完成:');
    console.log('- 追蹤按鈕:', document.querySelectorAll('.btn-follow').length, '個');
    console.log('- 購物車按鈕:', document.querySelectorAll('.btn-cart').length, '個');
}
...
// 更新所有追蹤按鈕的狀態
function updateFollowButtonStates() {
    document.querySelectorAll('.btn-follow').forEach(button => {
        const productCard = button.closest('.puff-card');
        if (!productCard) return;
        
        const productId = productCard.dataset.id;
        const isFollowing = wishlist.some(item => item.id === productId);
