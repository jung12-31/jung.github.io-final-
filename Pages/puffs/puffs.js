...
// 確保 showPuffNotification 也使用這個函數
...
// 初始化評論系統
function initPuffReviewSystem() {
    if (isInitialized) {
        console.log('泡芙專區評論系統已初始化，跳過重複初始化');
        return;
    }
    
    console.log('初始化泡芙專區評論系統');
    
    // 檢查是否有評論資料，如果沒有則創建預設資料
    initializePuffReviewsData();
    
    // 載入商品評論資料
    loadPuffProductReviews();
    
    // 設置評論按鈕事件
    setupPuffReviewButtons();
    
    // 初始化星星評分系統
    initPuffRatingModalStars();
    
    // 綁定提交按鈕
    bindPuffSubmitButton();
    
    isInitialized = true;
}

// 初始化泡芙專區評論資料
function initializePuffReviewsData() {
    const reviewsData = localStorage.getItem('chulinPuffReviews');
    
    if (!reviewsData) {
        console.log('沒有找到泡芙專區評論資料，創建預設資料');
        const defaultReviews = createPuffDefaultReviews();
        localStorage.setItem('chulinPuffReviews', JSON.stringify(defaultReviews));
        console.log('泡芙專區預設評論已創建:', defaultReviews);
    } else {
        console.log('已有泡芙專區評論資料:', JSON.parse(reviewsData));
    }
}

// 建立泡芙專區預設評論資料
function createPuffDefaultReviews() {
    return {
        '法式葡萄泡芙': {
            rating: 4.7,
            reviews: [
                { name: '張小姐', rating: 5, text: '葡萄和卡士達的搭配非常清爽，夏天吃超適合！', date: '2024-03-18' },
                { name: '李先生', rating: 5, text: '葡萄很新鮮，泡芙外皮酥脆，會再回購！', date: '2024-03-12' },
                { name: '王太太', rating: 4, text: '酸甜平衡做得很好，不會太甜膩。', date: '2024-03-08' }
            ]
        },
        '芒果泡芙': {
            rating: 4.9,
            reviews: [
                { name: '陳小姐', rating: 5, text: '芒果香氣濃郁，甜而不膩，家人搶著吃！', date: '2024-03-17' },
                { name: '林先生', rating: 5, text: '熱帶風味十足，夏天必吃的消暑甜點！', date: '2024-03-11' },
                { name: '吳太太', rating: 5, text: '芒果餡料給得很大方，每一口都很滿足。', date: '2024-03-06' }
            ]
        },
        '提拉米蘇泡芙': {
            rating: 4.6,
            reviews: [
                { name: '鄭小姐', rating: 5, text: '咖啡和可可的平衡做得很好，濃郁但不苦澀！', date: '2024-03-19' },
                { name: '周先生', rating: 4, text: '將提拉米蘇做成泡芙很有創意，口感層次豐富。', date: '2024-03-14' },
                { name: '黃太太', rating: 5, text: '成熟系口味，適合搭配咖啡或茶一起享用。', date: '2024-03-09' }
            ]
        },
        '黑糖珍珠泡芙': {
            rating: 4.8,
            reviews: [
                { name: '趙小姐', rating: 5, text: '黑糖香氣濃厚，珍珠Q彈有嚼勁，創新又美味！', date: '2024-03-20' },
                { name: '劉先生', rating: 5, text: '甜度控制得剛剛好，珍珠口感很特別。', date: '2024-03-15' },
                { name: '孫太太', rating: 4, text: '黑糖味道很香，珍珠保持Q彈不會硬。', date: '2024-03-10' }
            ]
        }
    };
}
...
// 載入泡芙專區商品評論資料到頁面
function loadPuffProductReviews() {
    try {
        const reviewsData = localStorage.getItem('chulinPuffReviews');
        if (!reviewsData) {
            console.error('沒有找到泡芙專區評論資料');
            return;
        }
        
        const reviews = JSON.parse(reviewsData);
        console.log('載入泡芙專區評論資料，共有', Object.keys(reviews).length, '個商品評論');
        
        // 獲取頁面上的所有泡芙商品卡片
        const puffCards = document.querySelectorAll('.puff-card');
        console.log('頁面上有', puffCards.length, '個泡芙商品卡片');
        
        puffCards.forEach((card, index) => {
            const productName = card.querySelector('.puff-name').textContent.trim();
            ...
            if (productReviews) {
                ...
                // 更新星星顯示
                if (ratingStars) {
                    updatePuffProductStarsDisplay(ratingStars, rating);
                }
                
                // 更新評論預覽
                if (productReviews.reviews && productReviews.reviews.length > 0) {
                    updatePuffReviewsPreview(card, productReviews.reviews);
                } else {
                    // 如果沒有評論，顯示暫無評論
                ...
                const ratingStars = card.querySelector('.rating-stars');
                if (ratingStars) {
                    updatePuffProductStarsDisplay(ratingStars, 0);
                }
            }
        });
        
        console.log('泡芙專區評論資料載入完成');
    } catch (error) {
        console.error('載入泡芙專區評論資料時發生錯誤:', error);
    }
}

// 更新泡芙專區評論預覽
function updatePuffReviewsPreview(card, reviews) {
    const previewContainer = card.querySelector('.reviews-preview');
            ...
            reviewItem.innerHTML = `
                <div class="reviewer-info">
                    ...
                    <div class="review-stars">
                        ${getPuffStarsHTML(review.rating)}
                    </div>
                </div>
...
// 獲取泡芙專區星星HTML
function getPuffStarsHTML(rating) {
    let starsHTML = '';
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
...
// 更新泡芙專區商品星星顯示
function updatePuffProductStarsDisplay(starsElement, rating) {
...
// 設置泡芙專區評論按鈕事件
function setupPuffReviewButtons() {
    console.log('設置泡芙專區評論按鈕事件...');
    
    // 為所有「我要評分」按鈕綁定事件
    const rateButtons = document.querySelectorAll('.puff-card .rate-product-btn');
    console.log('找到泡芙專區我要評分按鈕:', rateButtons.length);
    ...
    rateButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const card = this.closest('.puff-card');
            if (!card) {
                console.log('找不到泡芙卡片元素');
                return;
            }
            
            const productName = card.querySelector('.puff-name').textContent.trim();
            console.log('點擊泡芙專區我要評分:', productName);
            
            // 開啟評分模態框
            openPuffRatingModalWithAuth(productName);
    ...
    // 為所有「查看評論」按鈕綁定事件
    const viewAllButtons = document.querySelectorAll('.puff-card .view-all-reviews-btn');
    console.log('找到泡芙專區查看評論按鈕:', viewAllButtons.length);
    ...
    viewAllButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const productName = card.querySelector('.puff-name').textContent.trim();
            console.log('點擊泡芙專區查看評論:', productName);
            
            // 開啟評論模態框
            openPuffReviewsModal(productName);
...
// 開啟泡芙專區評分模態框（整合會員登入檢查）
function openPuffRatingModalWithAuth(productName) {
    console.log('開啟泡芙專區評分模態框:', productName);
    ...
    if (!isLoggedIn || !userData.email) {
        ...
        showPuffNotification('請先登入會員才能進行評分', 'error');
        ...
        if (authModal) {
            ...
            setTimeout(() => {
                ...
                localStorage.setItem('pendingPuffRatingProduct', productName);
            }, 10);
        }
        return;
    }
    ...
    openPuffRatingModal(productName);
}

// 初始化泡芙專區評分模態框的星星
function initPuffRatingModalStars() {
    ...
    stars.forEach(star => {
        ...
        star.addEventListener('mouseenter', function() {
            ...
            highlightPuffStars(stars, value);
            updatePuffRatingText(ratingText, value);
        });
        
        // 滑鼠移出事件 - 恢復實際評分
        star.addEventListener('mouseleave', function() {
            highlightPuffStars(stars, currentReviewRating);
            updatePuffRatingText(ratingText, currentReviewRating);
        });
    ...
    // 初始化顯示
    highlightPuffStars(stars, 0);
    updatePuffRatingText(ratingText, 0);
}

// 高亮泡芙專區星星
function highlightPuffStars(stars, rating) {
...
// 更新泡芙專區評分文字
function updatePuffRatingText(element, rating) {
    if (!element) return;
...
// 開啟泡芙專區評論模態框
function openPuffReviewsModal(productName) {
    ...
    console.log('開啟泡芙專區評論模態框:', productName);
    ...
    if (productReviews) {
        ...
        // 更新星星顯示
        if (ratingStarsLarge) {
            ratingStarsLarge.innerHTML = getPuffStarsHTML(productReviews.rating);
        }
...
// 關閉泡芙專區評論模態框
function closePuffReviewsModal() {
    const modal = document.getElementById('reviewsModal');
    if (modal) {
...
// 開啟泡芙專區評分模態框
function openPuffRatingModal(productName) {
...
function bindPuffSubmitButton() {
    ...
    if (submitBtn) {
        ...
        console.log('泡芙專區提交按鈕已重新綁定');
    }
}

// 提交泡芙專區評分
function submitPuffRating() {
    const modal = document.getElementById('ratingModal');
    if (!modal) {
    ...
    console.log('提交泡芙專區評分，產品:', productName, '評分:', currentReviewRating);
    
    // 驗證評分
    if (currentReviewRating === 0) {
        showPuffNotification('請選擇評分星等', 'error');
        return;
    }
    ...
    // 驗證評論內容
    if (!content) {
        showPuffNotification('請填寫評論內容', 'error');
        return;
    }
    ...
    try {
        ...
        // 獲取現有評論資料
        const reviewsData = localStorage.getItem('chulinPuffReviews');
        let reviews = reviewsData ? JSON.parse(reviewsData) : createPuffDefaultReviews();
        ...
        // 儲存到 localStorage
        localStorage.setItem('chulinPuffReviews', JSON.stringify(reviews));
        
        console.log('泡芙專區評論已儲存，更新後的商品資料:', reviews[productName]);
        
        // 更新UI
        loadPuffProductReviews();
        
        // 顯示成功訊息
        showPuffNotification('評論提交成功！感謝您的分享', 'success');
        
        // 關閉模態框
        closePuffRatingModal();
        
    } catch (error) {
        console.error('提交泡芙專區評分時發生錯誤:', error);
        showPuffNotification('提交評分時發生錯誤，請稍後再試', 'error');
    }
}
...
function showPuffNotification(message, type = 'info') {
    // 移除現有的通知
    const existingNotification = document.querySelector('.puff-notification');
    if (existingNotification) existingNotification.remove();
    
    // 創建通知元素
    const notification = document.createElement('div');
    notification.className = `puff-notification notification ${type}`;
...
// 檢查待處理的泡芙專區評分請求
function checkPendingPuffRating() {
    const pendingProduct = localStorage.getItem('pendingPuffRatingProduct');
    if (pendingProduct) {
        const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
        const userData = JSON.parse(localStorage.getItem('chulin_user')) || {};
        ...
        if (isLoggedIn && userData.email) {
            // 如果已登入，自動開啟評分模態框
            console.log('用戶已登入，自動開啟待處理的泡芙專區評分:', pendingProduct);
            setTimeout(() => {
                openPuffRatingModal(pendingProduct);
                // 清除待處理請求
                localStorage.removeItem('pendingPuffRatingProduct');
            }, 500);
        }
    }
}
...
// 修復泡芙專區模態框關閉功能
function fixPuffModalClose() {
    console.log('修復泡芙專區模態框關閉功能...');
...
// 添加泡芙專區通知樣式
function addPuffNotificationStyles() {
    if (!document.getElementById('puff-notification-styles')) {
        const style = document.createElement('style');
        style.id = 'puff-notification-styles';
        style.textContent = `
            .notification {
                position: fixed;
                top: 20px;
...
// ==================== 泡芙專區主初始化流程 ====================
function initializePuffPage() {
    console.log('泡芙專區頁面加載完成，開始初始化...');
    
    // 初始化泡芙專區評論資料
    initializePuffReviewsData();
    
    // 初始化泡芙專區評論系統
    initPuffReviewSystem();
    
    // 修復模態框關閉功能
    fixPuffModalClose();
    
    // 添加通知樣式
    addPuffNotificationStyles();
    
    // 檢查是否有待處理的評分請求
    checkPendingPuffRating();
    
    console.log('泡芙專區頁面初始化完成');
}

// ==================== DOM 載入完成後初始化泡芙專區 ====================
document.addEventListener('DOMContentLoaded', function() {
    console.log('泡芙專區頁面已載入');
...
// ==================== 調試函數 ====================
// 手動重置泡芙專區評論資料
function resetPuffReviews() {
    localStorage.removeItem('chulinPuffReviews');
    console.log('泡芙專區評論資料已重置');
    location.reload();
}

// 檢查泡芙專區當前狀態
function checkPuffReviewsStatus() {
    const reviewsData = localStorage.getItem('chulinPuffReviews');
    console.log('=== 泡芙專區評論系統狀態檢查 ===');
    console.log('LocalStorage 中是否有泡芙專區評論資料:', reviewsData ? '是' : '否');
    
    if (reviewsData) {
        const reviews = JSON.parse(reviewsData);
        console.log('所有泡芙商品鍵名:', Object.keys(reviews));
        
        // 檢查每個商品
        document.querySelectorAll('.puff-card').forEach((card, index) => {
            const productName = card.querySelector('.puff-name').textContent.trim();
...
// 將調試函數暴露到全局
window.resetPuffReviews = resetPuffReviews;
window.checkPuffReviewsStatus = checkPuffReviewsStatus;

// 全局函數，讓HTML可以呼叫
window.openPuffReviewsModal = openPuffReviewsModal;
window.closePuffReviewsModal = closePuffReviewsModal;
window.openPuffRatingModal = openPuffRatingModal;
window.closePuffRatingModal = closePuffRatingModal;
window.submitPuffRating = submitPuffRating;
...
function initProductButtons() {
    ...
    followBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const productName = card.querySelector('.puff-name').textContent .trim();
            const productPrice = parseInt(card.querySelector('.puff-price').textContent.replace('NT$ ', '').replace(',', ''));
...
function initProductIds() {
    const cards = document.querySelectorAll('.puff-card');
    cards.forEach((card, index) => {
        card.dataset.id = `puff_${index + 1}`;
    });
}
...
function toggleWishlist(productId, productName, productPrice, productImage) {
    ...
    const followBtn = document.querySelector(`.puff-card[data-id="${productId}"] .btn-follow`);
...
function updateFollowButtons() {
    const followBtns = document.querySelectorAll('.btn-follow');
    followBtns.forEach(btn => {
        const card = btn.closest('.puff-card');
        const productId = card.dataset.id;
...
function addMessage(text, sender) {
    ...
    const now = new Date();
    const timeString = `${now.getHours().toString().padSpuff(2, '0')}:${now.getMinutes().toString().padSpuff(2, '0')}`;
