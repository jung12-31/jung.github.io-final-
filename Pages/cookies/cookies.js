...
// ==================== 餅乾專區評論系統功能 ====================
let currentCookieReviewRating = 0;
let isCookieReviewInitialized = false;

// 初始化評論系統
function initCookieReviewSystem() {
    if (isCookieReviewInitialized) {
        console.log('餅乾專區評論系統已初始化，跳過重複初始化');
        return;
    }
    
    console.log('初始化餅乾專區評論系統');
    
    // 檢查是否有評論資料，如果沒有則創建預設資料
    initializeCookieReviewsData();
    
    // 載入商品評論資料
    loadCookieProductReviews();
    
    // 設置評論按鈕事件
    setupCookieReviewButtons();
    
    // 初始化星星評分系統
    initCookieRatingModalStars();
    
    // 綁定提交按鈕
    bindCookieSubmitButton();
    
    isCookieReviewInitialized = true;
}

// 初始化餅乾專區評論資料
function initializeCookieReviewsData() {
    const reviewsData = localStorage.getItem('chulinCookieReviews');
    
    if (!reviewsData) {
        console.log('沒有找到餅乾專區評論資料，創建預設資料');
        const defaultReviews = createCookieDefaultReviews();
        localStorage.setItem('chulinCookieReviews', JSON.stringify(defaultReviews));
        console.log('餅乾專區預設評論已創建:', defaultReviews);
    } else {
        console.log('已有餅乾專區評論資料:', JSON.parse(reviewsData));
    }
}

// 建立餅乾專區預設評論資料
function createCookieDefaultReviews() {
    return {
        '巧克力豆餅乾': {
            rating: 4.5,
            reviews: [
                { name: '丁先生', rating: 4, text: '巧克力豆分佈得很均勻，外層酥脆內裡柔軟，甜度適中，是小朋友的最愛。', date: '2024-05-15' },
                { name: '余小姐', rating: 5, text: '濃醇的巧克力香氣很迷人，是一款完全不會吃膩的經典口味，非常推。', date: '2024-05-16' },
                { name: '許太太', rating: 5, text: '巧克力豆給得很大方，每一口都能吃到巧克力，甜度控制得剛剛好。', date: '2024-05-12' }
            ]
        },
        '曲奇餅乾': {
            rating: 4.8,
            reviews: [
                { name: '何小姐', rating: 5, text: '奶油香氣超溫潤，真的是入口即化，沒有多餘的添加物感，很純粹的小點心。', date: '2024-05-18' },
                { name: '蔡先生', rating: 5, text: '酥鬆的口感拿捏得剛剛好，配咖啡或配牛奶都非常適合，一盒很快就吃光了。', date: '2024-05-19' },
                { name: '高太太', rating: 4, text: '奶香味濃郁，入口即化的口感很特別，配茶吃很適合。', date: '2024-05-14' }
            ]
        },
        '燕麥餅乾': {
            rating: 4.7,
            reviews: [
                { name: '方小姐', rating: 4, text: '燕麥顆粒帶來很棒的咀嚼感，口感紮實且清爽不膩，是想吃甜點時的低負擔選擇。', date: '2024-05-21' },
                { name: '江先生', rating: 5, text: '紮實但不乾硬，每一口都吃得到燕麥的穀物香氣，感覺非常健康又美味。', date: '2024-05-22' },
                { name: '謝小姐', rating: 5, text: '燕麥的香氣很足，不會太甜，適合當作早餐或下午茶的點心。', date: '2024-05-17' }
            ]
        },
        '美式軟餅乾': {
            rating: 4.9,
            reviews: [
                { name: '蕭小姐', rating: 5, text: '外酥內軟的口感太銷魂了！每一口都有剛出爐般的溫暖幸福感，甜度也很剛好。', date: '2024-05-24' },
                { name: '沈先生', rating: 5, text: '厚實的口感吃起來很過癮，巧克力和餅乾體的比例完美，是一款很有質感的軟餅乾。', date: '2024-05-25' },
                { name: '蘇太太', rating: 5, text: '軟餅乾的口感真的很特別，外層微脆內裡柔軟，搭配熱茶或咖啡超棒！', date: '2024-05-20' }
            ]
        }
    };
}
...
// 載入餅乾專區商品評論資料到頁面
function loadCookieProductReviews() {
    try {
        const reviewsData = localStorage.getItem('chulinCookieReviews');
        if (!reviewsData) {
            console.error('沒有找到餅乾專區評論資料');
            return;
        }
        
        const reviews = JSON.parse(reviewsData);
        console.log('載入餅乾專區評論資料，共有', Object.keys(reviews).length, '個商品評論');
        
        // 獲取頁面上的所有餅乾商品卡片
        const cookieCards = document.querySelectorAll('.cookie-card');
        console.log('頁面上有', cookieCards.length, '個餅乾商品卡片');
        
        cookieCards.forEach((card, index) => {
            const productName = card.querySelector('.cookie-name').textContent.trim();
            ...
            if (productReviews) {
                ...
                // 更新星星顯示
                if (ratingStars) {
                    updateCookieProductStarsDisplay(ratingStars, rating);
                }
                
                // 更新評論預覽
                if (productReviews.reviews && productReviews.reviews.length > 0) {
                    updateCookieReviewsPreview(card, productReviews.reviews);
                } else {
                    // 如果沒有評論，顯示暫無評論
                ...
                const ratingStars = card.querySelector('.rating-stars');
                if (ratingStars) {
                    updateCookieProductStarsDisplay(ratingStars, 0);
                }
            }
        });
        
        console.log('餅乾專區評論資料載入完成');
    } catch (error) {
        console.error('載入餅乾專區評論資料時發生錯誤:', error);
    }
}

// 更新餅乾專區評論預覽
function updateCookieReviewsPreview(card, reviews) {
    const previewContainer = card.querySelector('.reviews-preview');
            ...
            reviewItem.innerHTML = `
                <div class="reviewer-info">
                    ...
                    <div class="review-stars">
                        ${getCookieStarsHTML(review.rating)}
                    </div>
                </div>
...
// 獲取餅乾專區星星HTML
function getCookieStarsHTML(rating) {
    let starsHTML = '';
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
...
// 更新餅乾專區商品星星顯示
function updateCookieProductStarsDisplay(starsElement, rating) {
...
// 設置餅乾專區評論按鈕事件（修正版）
function setupCookieReviewButtons() {
    console.log('設置餅乾專區評論按鈕事件...');
    
    // 為所有「我要評分」按鈕綁定事件
    const rateButtons = document.querySelectorAll('.cookie-card .rate-product-btn');
    console.log('找到餅乾專區我要評分按鈕:', rateButtons.length);
    ...
    rateButtons.forEach(button => {
        ...
        // 重新綁定事件 - 移除 stopPropagation
        newButton.addEventListener('click', function(e) {
            e.preventDefault(); // 只阻止預設行為
            
            const card = this.closest('.cookie-card');
            if (!card) {
                console.log('找不到餅乾卡片元素');
                return;
            }
            
            const productName = card.querySelector('.cookie-name').textContent.trim();
            console.log('點擊餅乾專區我要評分:', productName);
            
            // 開啟評分模態框
            openCookieRatingModalWithAuth(productName);
        });
    });
    
    // 為所有「查看評論」按鈕綁定事件
    const viewAllButtons = document.querySelectorAll('.cookie-card .view-all-reviews-btn');
    console.log('找到餅乾專區查看評論按鈕:', viewAllButtons.length);
    ...
    viewAllButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function(e) {
            ...
            const productName = card.querySelector('.cookie-name').textContent.trim();
            console.log('點擊餅乾專區查看評論:', productName);
            
            // 開啟評論模態框
            openCookieReviewsModal(productName);
        });
    });
}

// 開啟餅乾專區評分模態框（整合會員登入檢查）
function openCookieRatingModalWithAuth(productName) {
    console.log('開啟餅乾專區評分模態框:', productName);
    ...
    if (!isLoggedIn || !userData.email) {
        ...
        showNotification('請先登入會員才能進行評分', 'error');
        ...
        if (authModal) {
            ...
            setTimeout(() => {
                ...
                localStorage.setItem('pendingCookieRatingProduct', productName);
            }, 10);
        }
        return;
    }
    ...
    openCookieRatingModal(productName);
}

// 初始化餅乾專區評分模態框的星星
function initCookieRatingModalStars() {
    ...
    // 重置評分
    currentCookieReviewRating = 0;
    
    stars.forEach(star => {
        // 滑鼠移入事件 - 預覽效果
        star.addEventListener('mouseenter', function() {
            const value = parseInt(this.dataset.value);
            highlightCookieStars(stars, value);
            updateCookieRatingText(ratingText, value);
        });
        
        // 滑鼠移出事件 - 恢復實際評分
        star.addEventListener('mouseleave', function() {
            highlightCookieStars(stars, currentCookieReviewRating);
            updateCookieRatingText(ratingText, currentCookieReviewRating);
        });
        
        // 點擊事件 - 設定評分 - 移除 stopPropagation
        star.addEventListener('click', function() {
            currentCookieReviewRating = parseInt(this.dataset.value);
            console.log('選擇評分:', currentCookieReviewRating);
            highlightCookieStars(stars, currentCookieReviewRating);
            updateCookieRatingText(ratingText, currentCookieReviewRating);
        });
    });
    ...
    // 初始化顯示
    highlightCookieStars(stars, 0);
    updateCookieRatingText(ratingText, 0);
}

// 高亮餅乾專區星星
function highlightCookieStars(stars, rating) {
...
// 更新餅乾專區評分文字
function updateCookieRatingText(element, rating) {
    if (!element) return;
...
// 開啟餅乾專區評論模態框
function openCookieReviewsModal(productName) {
    ...
    console.log('開啟餅乾專區評論模態框:', productName);
    ...
    if (productReviews) {
        ...
        // 更新星星顯示
        if (ratingStarsLarge) {
            ratingStarsLarge.innerHTML = getCookieStarsHTML(productReviews.rating);
        }
...
// 關閉餅乾專區評論模態框
function closeCookieReviewsModal() {
    const modal = document.getElementById('reviewsModal');
    if (modal) {
...
// 開啟餅乾專區評分模態框
function openCookieRatingModal(productName) {
...
// 關閉餅乾專區評分模態框
function closeCookieRatingModal() {
    const modal = document.getElementById('ratingModal');
    if (modal) {
        modal.classList.remove('active');
        setTimeout(() => {
...
function bindCookieSubmitButton() {
    ...
    if (submitBtn) {
        ...
        console.log('餅乾專區提交按鈕已重新綁定');
    }
}

// 提交餅乾專區評分
function submitCookieRating() {
    const modal = document.getElementById('ratingModal');
    if (!modal) {
    ...
    console.log('提交餅乾專區評分，產品:', productName, '評分:', currentCookieReviewRating);
    
    // 驗證評分
    if (currentCookieReviewRating === 0) {
        showNotification('請選擇評分星等', 'error');
        return;
    }
    ...
    // 驗證評論內容
    if (!content) {
        showNotification('請填寫評論內容', 'error');
        return;
    }
    ...
    try {
        ...
        // 獲取現有評論資料
        const reviewsData = localStorage.getItem('chulinCookieReviews');
        let reviews = reviewsData ? JSON.parse(reviewsData) : createCookieDefaultReviews();
        ...
        if (existingReviewIndex !== -1) {
            ...
            newReview = {
                ...
                userName: userName,
                userEmail: userEmail,
                rating: currentCookieReviewRating,
        ...
        // 儲存到 localStorage
        localStorage.setItem('chulinCookieReviews', JSON.stringify(reviews));
        
        console.log('餅乾專區評論已儲存，更新後的商品資料:', reviews[productName]);
        
        // 更新UI
        loadCookieProductReviews();
        
        // 顯示成功訊息
        showNotification('評論提交成功！感謝您的分享', 'success');
        
        // 關閉模態框
        closeCookieRatingModal();
        
    } catch (error) {
        console.error('提交餅乾專區評分時發生錯誤:', error);
        showNotification('提交評分時發生錯誤，請稍後再試', 'error');
    }
}

// 檢查待處理的餅乾專區評分請求
function checkPendingCookieRating() {
    const pendingProduct = localStorage.getItem('pendingCookieRatingProduct');
    if (pendingProduct) {
        const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
        const userData = JSON.parse(localStorage.getItem('chulin_user')) || {};
        ...
        if (isLoggedIn && userData.email) {
            // 如果已登入，自動開啟評分模態框
            console.log('用戶已登入，自動開啟待處理的餅乾專區評分:', pendingProduct);
            setTimeout(() => {
                openCookieRatingModal(pendingProduct);
                // 清除待處理請求
                localStorage.removeItem('pendingCookieRatingProduct');
            }, 500);
        }
    }
}
...
// 修復餅乾專區模態框關閉功能
function fixCookieModalClose() {
    console.log('修復餅乾專區模態框關閉功能...');
    ...
    if (ratingModalClose) {
        ...
        // 重新綁定事件 - 移除 stopPropagation
        ratingModalClose.addEventListener('click', function() {
            closeCookieRatingModal();
        });
    }
...
function initializeCookiePage() {
    console.log('餅乾專區頁面加載完成，開始初始化...');
    
    // 初始化餅乾專區評論資料
    initializeCookieReviewsData();
    
    // 初始化餅乾專區評論系統
    initCookieReviewSystem();
    
    // 修復模態框關閉功能
    fixCookieModalClose();
    
    // 檢查是否有待處理的評分請求
    checkPendingCookieRating();
    
    console.log('餅乾專區頁面初始化完成');
}

// ==================== DOM 載入完成後初始化餅乾專區 ====================
document.addEventListener('DOMContentLoaded', function() {
    console.log('餅乾專區頁面已載入');
...
function initializeAllFeatures() {
    ...
    // 初始化右上角按鈕（修正版 - 移除 stopPropagation）
    initTopRightButtons();
...
// ==================== 調試函數 ====================
// 手動重置餅乾專區評論資料
function resetCookieReviews() {
    localStorage.removeItem('chulinCookieReviews');
    console.log('餅乾專區評論資料已重置');
    location.reload();
}

// 檢查餅乾專區當前狀態
function checkCookieReviewsStatus() {
    const reviewsData = localStorage.getItem('chulinCookieReviews');
    console.log('=== 餅乾專區評論系統狀態檢查 ===');
    console.log('LocalStorage 中是否有餅乾專區評論資料:', reviewsData ? '是' : '否');
    
    if (reviewsData) {
        const reviews = JSON.parse(reviewsData);
        console.log('所有餅乾商品鍵名:', Object.keys(reviews));
...
// 將調試函數暴露到全局
window.resetCookieReviews = resetCookieReviews;
window.checkCookieReviewsStatus = checkCookieReviewsStatus;

// 全局函數，讓HTML可以呼叫
window.openCookieReviewsModal = openCookieReviewsModal;
window.closeCookieReviewsModal = closeCookieReviewsModal;
window.openCookieRatingModal = openCookieRatingModal;
window.closeCookieRatingModal = closeCookieRatingModal;
window.submitCookieRating = submitCookieRating;
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            setTimeout(() => {
                ...
                const pendingProduct = localStorage.getItem('pendingCookieRatingProduct');
                if (pendingProduct) {
                    console.log('登入成功，自動開啟待處理的餅乾專區評分:', pendingProduct);
                    setTimeout(() => {
                        openCookieRatingModal(pendingProduct);
                        // 清除待處理請求
                        localStorage.removeItem('pendingCookieRatingProduct');
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', function(e) {
            ...
            setTimeout(() => {
                ...
                const pendingProduct = localStorage.getItem('pendingCookieRatingProduct');
                if (pendingProduct) {
                    console.log('註冊成功，自動開啟待處理的餅乾專區評分:', pendingProduct);
                    setTimeout(() => {
                        openCookieRatingModal(pendingProduct);
                        // 清除待處理請求
                        localStorage.removeItem('pendingCookieRatingProduct');
...
function initProductButtons() {
    ...
    followBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const productName = card.querySelector('.cookie-name').textContent.trim();
            const productPrice = parseInt(card.querySelector('.cookie-price').textContent.replace('NT$ ', '').replace(',', ''));
...
function initProductIds() {
    ...
    cards.forEach((card, index) => {
        card.dataset.id = `cookie_${index + 1}`;
    });
}
...
function toggleWishlist(productId, productName, productPrice, productImage) {
    ...
    const followBtn = document.querySelector(`.cookie-card[data-id="${productId}"] .btn-follow`);
...
// 右上角按鈕功能（修正版 - 移除 stopPropagation）
function initTopRightButtons() {
    console.log('初始化右上角按鈕功能');
    
    // 聊天按鈕 - 移除 stopPropagation
