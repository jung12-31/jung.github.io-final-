...
function initTopRightButtons() {
    ...
    if (profileBtn) {
        ...
        // 添加點擊測試
        console.log('會員按鈕元素詳細資訊:', {
            id: profileBtn.id,
            tagName: profileBtn.tagName,
            className: profileBtn.className,
            innerHTML: profileBtn.innerHTML,
            hasClickListener: true
        });
    } else {
        console.error('找不到會員按鈕元素!');
    }
}
...
function showUserProfile() {
    ...
    if (!document.querySelector('#profile-popup-styles')) {
...
function removeFromCart(productId) {
    ...
    if (productIndex !== -1) {
        ...
        updateCart();
        showNotification(`已從購物車移除 ${productName}`, 'info');
    }
}
...
function toggleWishlist(productId, productName, productPrice, productImage) {
    ...
    const followBtn = document.querySelector(`.puff-card[data-id="${productId}"] .btn-follow, .product-card[data-id="${productId}"] .btn-follow`);
...
function updateFollowButtons() {
    ...
    followBtns.forEach(btn => {
        const card = btn.closest('.puff-card, .product-card');
        if (!card) return;
...
function initProductButtons() {
    // 為每個商品卡片添加點擊事件
    const productCards = document.querySelectorAll('.product-card, .puff-card'); // 支援兩種卡片類型
    productCards.forEach((card, index) => {
        ...
        card.addEventListener('click', function(e) {
            ...
            // 取得商品資訊
            const productId = card.dataset.id || `product_${index + 1}`;
            const productName = card.querySelector('h4')?.textContent || card.querySelector('h3')?.textContent || '商品名稱';
            
            // 取得價格 - 處理不同頁面的價格選擇器
            const priceElement = card.querySelector('.price') || card.querySelector('.card-price') || card.querySelector('.puff-price');
            let productPrice = 0;
            if (priceElement) {
                const priceText = priceElement.textContent.replace('NT$ ', '').replace(',', '');
        ...
        // 初始化商品ID
        if (!card.dataset.id) {
            card.dataset.id = `product_${index + 1}`;
        }
    ...
    followBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const card = this.closest('.product-card, .puff-card');
            const productId = card.dataset.id;
            const productName = card.querySelector('h4')?.textContent || card.querySelector('h3')?.textContent || '商品名稱';
...
// 創建快速操作彈出層
function createQuickActions(card, product) {
    // 移除現有的彈出層
    ...
    const isTracked = wishlist.some(item => item.id === product.id);
    
    popup.innerHTML = `
        <div class="popup-content">
            <h4>${product.name}</h4>
            <p class="popup-price">NT$ ${product.price.toLocaleString()}</p>
            <div class="popup-actions">
                <button class="popup-btn add-to-cart-btn">
                    <i class="fas fa-shopping-cart"></i> 加入購物車
                </button>
                <button class="popup-btn ${isTracked ? 'tracked' : ''}" id="toggleTrackBtn">
                    <i class="fas fa-heart"></i> ${isTracked ? '已追蹤' : '追蹤商品'}
                </button>
                <button class="popup-btn close-popup">
    ...
    const toggleTrackBtn = popup.querySelector('#toggleTrackBtn');
    ...
    toggleTrackBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        toggleWishlist(product.id, product.name, product.price, product.image);
        
        // 更新按鈕狀態
        const isNowTracked = wishlist.some(item => item.id === product.id);
        if (isNowTracked) {
            toggleTrackBtn.innerHTML = '<i class="fas fa-heart" style="color: #ff4757;"></i> 已追蹤';
            toggleTrackBtn.classList.add('tracked');
        } else {
            toggleTrackBtn.innerHTML = '<i class="fas fa-heart"></i> 追蹤商品';
            toggleTrackBtn.classList.remove('tracked');
        }
    });
        ...
        chatInput.addEventListener('keypress', (e) => {
            if(e.key === 'Enter') {
                ...
                if (val.trim()) {
                    ...
                    setTimeout(() => {
                        if(chatStep === 1) {
                            addMessage('了解，已標註您的情況，專員正在接入中，請稍候。', 'sys');
                            chatStep = 0;
                        } else if(chatStep === 2) {
                            // 訂單毀損選項
                            addMessage('了解您的情況，已為您建立客訴案件。請提供您的訂單編號，我們將優先處理您的問題。', 'sys');
                            chatStep = 0;
                        } else if(chatStep === 3) {
                            // 內容修改選項
                            addMessage('了解您的需求，請提供訂單編號及要修改的內容，我們將為您處理。', 'sys');
                            chatStep = 0;
                        } else {
                            addMessage('邱林助手收到您的訊息了，請問還有其他能幫您的嗎？', 'sys');
                        }
                    }, 1000);
                }
            }
...
window.connectToAgent = function() {
    showNotification('正在為您聯繫專人客服，請稍候...');
    setTimeout(() => {
        showNotification('專員將在1分鐘內與您聯繫', 'success');
    }, 1500);
};

// 打開郵件表單
window.openEmailForm = function() {
    showNotification('請寄送郵件至 service@chulin.com，我們會在48小時內回覆您！');
};
...
window.debugLogin = function() {
    ...
    console.log('- localStorage.chulin_user:', localStorage.getItem('chulin_user'));
    console.log('- 會員按鈕:', document.getElementById('profileBtn'));
    
    // 模擬點擊會員按鈕
    const profileBtn = document.getElementById('profileBtn');
    if (profileBtn) {
        console.log('模擬點擊會員按鈕');
        profileBtn.click();
    }
};